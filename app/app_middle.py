%%writefile app_middle.py

"""
Middle 버전 항공권 환불 상담 RAG 챗봇
- chunk_size: 1200
- chunk_overlap: 200
- 동의어 사전: 일부 확장 (30개)
- 8개 md 파일 (문서 최적화)
...
# 페이지 설정
st.set_page_config(page_title="Middle - 항공권 환불 챗봇", layout="wide")
st.title("✈️ Middle 버전 - 환불 상담 챗봇")
st.markdown("### 🟡 문서 최적화 버전")
st.caption("chunk_size=1200, overlap=200, 동의어 30개 (중간 개선)")

# 동의어 사전 (Middle - 일부 확장)
SYNONYM_DICT = {
    # 노쇼 관련 (핵심만)
    "노쇼": ["노쇼", "No-Show", "no-show", "예약부도"],
    "no-show": ["노쇼", "No-Show", "no-show", "예약부도"],
    "예약부도": ["노쇼", "예약부도", "no-show"],
...
with st.sidebar:
    ...
    # 개선 정보
    st.divider()
    with st.expander("🔍 Middle 개선사항"):
        st.caption("• 동의어 사전: 30개")
        st.caption("• Chunk 크기: 800 → 1200")
        st.caption("• 문서 최적화: 일부 완료")
        st.caption("⚠️ 표 잘림 현상 일부 남음")
...
3. 표 형식의 정보가 있다면 가능한 한 포함해줘
4. 답변 마지막에 "⚠️ 정확한 정보는 항공사 공식 웹사이트를 확인해주세요"라고 안내해줘
...
def initialize_vectordb():
    """Middle 설정으로 벡터 DB 초기화"""

    # MD 파일 경로
    md_path = Path("/content/middle")
    ...
    # Middle 설정으로 청킹 (중간 크기)
    splitter = RecursiveCharacterTextSplitter(
        chunk_size=1200,  # Middle: 중간 크기 (표 일부 개선)
        chunk_overlap=200
    )
    ...
    db = Chroma.from_documents(
        ...
        collection_name="middle_chatbot"
    )
...
# 라우팅 키워드 (Middle - 확장)
RAG_KEYWORDS = [
    "환불", "refund", "취소", "cancel", "변경", "change",
    "수수료", "fee", "위약금", "penalty",
    "노쇼", "no-show", "예약부도",
    "FLEX", "BASIC", "STANDARD", "SAVER",
...
# 하단 정보
st.markdown("---")
st.caption("🟡 Middle 버전 - 문서 최적화")
st.caption("특징: chunk_size=1200, 동의어 30개, 문서 일부 최적화")
st.caption("⚠️ 표 잘림 현상 일부 남아있음 (After에서 완전 해결)")
